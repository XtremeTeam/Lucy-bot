...
register_command_handler(handler_poke, COMM_PREFIX+'poke', ['fun','all','*','poke'], 10, 'Poke the user. Forces him to pay attention to you /in chat, специально для слоупоков.\nlast10 instead of a nick show a list of workers who poked latest.', COMM_PREFIX+'poke <nick>|<parameter>', [COMM_PREFIX+'poke qwerty',COMM_PREFIX+'poke + sing %s',COMM_PREFIX+'poke - 2',COMM_PREFIX+'poke *'])
register_command_handler(handler_poke_add, COMM_PREFIX+'poke+', ['fun','all','*','poke'], 20, 'Add a custom phrases. The variable %s in the phrase refers to a place to insert a nickname (mandatory parameter). The phrase should be written by a third person, it will use the following form "/me your phrase". max number of custom phrases is 20 characters.', COMM_PREFIX+'poke+ <phrase>', [COMM_PREFIX+'poke+ sing %s'])
register_command_handler(handler_poke_del, COMM_PREFIX+'poke-', ['fun','all','*','poke'], 20, 'Delete a custom phrase. Write the number of phrase to erase the words, the bot will delete it permanently. Write the commands "poked*" to view the list. In order to delete all phrases just specify "*" instead of phrase number.', COMM_PREFIX+'poke- <number>', [COMM_PREFIX+'poke- 5',COMM_PREFIX+'poke- *'])
register_command_handler(handler_poke_list, COMM_PREFIX+'poke*', ['fun','all','*','poke'], 20, 'Displays a list of all custom phrases and its number.', COMM_PREFIX+'poke*', [COMM_PREFIX+'poke*'])
register_command_handler(handler_test, COMM_PREFIX+'test', ['fun','info','all'], 0, 'Check the bot, passed simply answers!', COMM_PREFIX+'test', [COMM_PREFIX+'test'])
register_command_handler(handler_clean_conf, COMM_PREFIX+'clean', ['fun','muc','all','*'], 15, 'Clean current conference (with null character).', COMM_PREFIX+'clean', [COMM_PREFIX+'clean'])
register_command_handler(handler_afools_control, COMM_PREFIX+'afools', ['fun','muc','all','*'], 30, 'Enables and disables the bots jokes, which the bot sometimes substitutes (command is always executed!) Standard response of commands.', COMM_PREFIX+'afools <1|0>', [COMM_PREFIX+'afools 1',COMM_PREFIX+'afools 0'])
#register_command_handler(invite_join)
register_command_handler(hnd_ivite_block, COMM_PREFIX+'inviteblock', ['все','мод','суперадмин'], 40, 'Добавляет/удаляет запрет на вход бота в определенную комнату по инвайту.Без параметров показывает список.', 'антиджойн <комната>', ['антиджойн уг@conference.jabber.ru'])
#The listed of below command handler are not recommended
register_command_handler(handler_kick_ass, COMM_PREFIX+'spam', ['fun','superadmin','muc','all','*'], 31, 'Spamming a JID in roster or a nick current conference with smiles ( :) ).\nTarget of spam is determine by first parameter <nick>.\nRepetition of spam is determine by the second parameter <amount>.\nThe amount of spam determined by third parameter <amount>.\nWrite this command in private.', COMM_PREFIX+'spam <nick> <amount> <amount>', [COMM_PREFIX+'spam guy@jsmart.web.id 50 10',COMM_PREFIX+'spam guy 100 8'])
