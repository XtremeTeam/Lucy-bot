...
register_command_handler(handler_disco, COMM_PREFIX+'disco', ['muc','info','all','*'], 10, 'Shows results of the review services for the specified JID.\nIt is also possible to browse on point (node). Request format jid#node.\nSecond or third (if the limiter is also given number of) option - search. Searches for a given word in the JID and description disco. If the search word to specify the name of the conference until the server name (example qwerty@), it will place the conference in overall rankings.\nIn the general chat can give max 50 results, without specifying the count - 10.\nIn private may give max 250, without specifying count 50.', COMM_PREFIX+'disco <server> <count results> <search string>', [COMM_PREFIX+'disco smart.web.id',COMM_PREFIX+'disco conference.jsmart.web.id 5',COMM_PREFIX+'disco conference.jsmart.web.id qwerty',COMM_PREFIX+'disco conference.jsmart.web.id 5 qwerty',COMM_PREFIX+'disco conference.jsmart.web.id qwerty@', COMM_PREFIX+'disco jsmart.web.id#services'])