...
register_command_handler(handler_wtf, 'wtf', ['info','wtf','all','*'], 11, 'Conclusion of the article from the database. Entirely, if the article is small, or the first page size %s characters, by default, if the article is great. It also allows you to receive and the rest of the page and set the page size in symbols, if the article is more than %s characters.' %(DEFLIMIT,DEFLIMIT), 'wtf [<page_number>[<page_size>[k]]] <article_name>', ['wtf article', 'wtf 3 article', 'wtf 44 article', 'wtf 3 10k big article', 'wtf 5 1000 big article'])
register_command_handler(handler_next, 'next', ['info','wtf','all','*'], 11, 'Lets get the following page of the article. Without arguments displays the following page article from the database after the withdrawal of the command %swtf, if the page size is smaller than the article and do not specify a step. When you specify a step displays a page number equal to the current page number + step. When referring to title of article, it becomes the current, i.e at the next command %snext will see the following page of this article.' , 'next [<article_name>]|[<step>]', ['next','next article', 'next 2'])
register_command_handler(handler_prev, 'prev', ['info','wtf','all','*'], 11, 'Lets get the previous page of the article. Without arguments displays the previous page article from the database, after the withdrawal of the command %swtf, if the page number is smaller than the article and not specified step. When you specify a step displays a page number equal to the number of the current page - step.  When referring to title of article, it becomes the current, i.e at the next command %sprev see previous page of this article.' , 'prev [<article_name>]|[<step>]', ['prev','prev article', 'prev 3'])
register_command_handler(handler_list, 'list', ['info','wtf','all','*'], 11, 'Lets get a list of articles, in whole or in parts. Without arguments displays the entire list of articles if the articles less %s, or part of the list size %s names, if more articles %s.' % (LISTLIMIT,LISTLIMIT,LISTLIMIT), 'list [<page>][<number>]', ['list', 'list 3', 'list 2 20'])
register_command_handler(handler_books, 'books', ['info','wtf','all','*'], 11, 'Lets get a list of books, in whole or in part. Without parameters displays the entire list of books if the book is less %s, or part of the list size %s names, if more books %s.' % (LISTLIMIT,LISTLIMIT,LISTLIMIT), 'books [<page>][<number>]', ['books', 'books 3', 'books 2 20'])
register_command_handler(handler_stat, 'stat', ['info','wtf','all','*'], 11, 'Lets get statistics on the open books. When you specify a negative integer closes the book with this number.', 'stat [-<number>]', ['stat','stat -3'])
register_command_handler(handler_dfn, 'dfn', ['info','wtf','all','*'], 11, ' Adds a new, or update existing, article. When you specify instead of article path to a text file with, adds an article from a text file.', 'dfn <name>=<body_articles>', ['dfn first article=article, such article'])
register_command_handler(handler_del, 'del', ['info','wtf','all','*'], 20, 'Deletes an article with the specified name from the dictionary.', 'del <name>', ['del first article'])
register_command_handler(handler_rnd, 'rnd', ['info','wtf','all','*'], 11, 'Displays a random article from the dictionary.', 'rnd', ['rnd'])
register_command_handler(handler_find, 'find', ['info','wtf','all','*'], 11, 'Looking for a word or phrase in the title and text of articles.', 'find <word>|<phrase>', ['find life','find something interesting'])
register_command_handler(handler_search, 'search', ['info','wtf','all','*'], 11, 'Looking for a word or phrase in the text of the article and displays the page on which the precise word or phrase.', 'search <article>:<word>|<phrase>', ['search book: riddle'])
register_command_handler(handler_count, 'count', ['info','wtf','all','*'], 11, 'Displays the number of articles in the dictionary.', 'count', ['count'])
register_command_handler(handler_get_wtf, 'get_wtf', ['info','wtf','all','*'], 11, 'Send an article from the dictionary file via jabber.', 'get_wtf <article_name>', ['get_wtf book'])
